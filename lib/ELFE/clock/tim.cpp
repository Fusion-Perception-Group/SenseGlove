...
namespace stm32 {
    namespace clock {
        namespace tim {
             ...
             */
            VoidResult<> BasicTimer::set_time_base(const TimeBaseConfig& config) const
            {
                enable_clock(*this);
                ...
                set_reload_buffered(config.auto_reload_buffered);

                auto r = set_auto_reload(period);
                ELFE_PROP(r, r);
            ...
            void BasicTimer::set_clock_source(const ClockSourceConfig& config) const noexcept
            {
                ...
                if (config.source == ClockSource::Internal) {
                } else if (config.source == ClockSource::ExternalMode1 or config.source == ClockSource::ExternalMode2) {
                    ...
                    if (config.source == ClockSource::ExternalMode1)
                        ...
                        tmp |= TIM_SMCR_ECE;
                } else if (config.source == ClockSource::TTI1FP1 or config.source == ClockSource::TTI1FP1_ED) {
                    reg.CCER = ((reg.CCER & ~(TIM_CCER_CC1P | TIM_CCER_CC1NP | TIM_CCER_CC1E))
                    ...
                    reg.CCER = ((reg.CCER & ~(TIM_CCER_CC2P | TIM_CCER_CC2NP | TIM_CCER_CC2E))
            ...
            VoidResult<> BasicTimer::init() const
            {
                enable_clock(*this);
                auto r = set_time_base(TimeBaseConfig());
                ELFE_PROP(r, r);

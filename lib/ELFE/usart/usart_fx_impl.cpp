...
#if defined(_ELFE_STM32FX) && !_ELFE_STM32_USE_GENERIC
    ...
    namespace usart {
        ...
        float HardUsart::_BaudRate::getter() const noexcept
        {
            ...
            if (auto mode = owner.get_extended_mode();
                mode == HardUsart::ExtendedMode::SmartCard or mode == HardUsart::ExtendedMode::LIN or mode == HardUsart::ExtendedMode::IrDA)
        ...
        VoidResult<> HardUsart::init() noexcept
        {
            clock::rcc::enable_clock(*this);
            set_extended_mode(ExtendedMode::None);
            set_sync_clock(false);
            auto r = set_word_length(WordLength::Bits8);
            ELFE_PROP(r, r);
            r = set_parity(Parity::None);
            ELFE_PROP(r, r);
            baudrate = 9600;
        ...
        VoidResult<> HardUsart::set_parity(Parity parity) noexcept
        {
        ...
        VoidResult<> HardUsart::set_stop_bits(StopBits stop_bits) noexcept
        {
        ...
        VoidResult<> HardUsart::set_word_length(WordLength word_length) noexcept
        {
        ...
        VoidResult<> HardUsart::break_transmission() noexcept
        {
            reg.CR1 |= USART_CR1_SBK;
            return EC::None;
        }
        Result<size_t> HardUsart::buffered_exchange_bytes(const void* send, size_t send_size, void* recv, size_t recv_size)
        {
            ELFE_ERROR_IF(
                not _buffer_enabled,
                Result<size_t>(0, EC::LogicError),
                std::logic_error("Buffer is not enabled"));
            ...
            while (send_size or recv_size) {
                ...
                if (timeout_us) {
                    auto r = checker.raise_if_timedout();
                    ELFE_PROP(r, Result<size_t>(0, r.error));
                }
        ...
        Result<size_t> HardUsart::sync_exchange_bytes(const void* send, size_t send_size, void* recv, size_t recv_size)
        {
            size_t sent_count = 0;
            size_t recv_count = 0;
            ...
            if ((get_word_length() == WordLength::Bits9) ^ // 8 bit mode
                ...
                while (sent_count + recv_count < send_size + recv_size) {
                    ...
                    do {
                        ...
                        auto r = raise_if_error();
                        ELFE_PROP(r, Result<size_t>(sent_count + recv_count, r.error));
                        if (timeout_us) {
                            r = checker.raise_if_timedout();
                            ELFE_PROP(r, Result<size_t>(sent_count + recv_count, r.error));
                        }
         ...
         */
        VoidResult<> HardUsart::set_baudrate(float baudrate_f, bool keep_oversampling) const
        {
            ...
            if (baudrate_f > pclk / 16) {
                if (keep_oversampling || is_oversample_8x()) {
                    ELFE_ERROR_IF(
                        baudrate_f > pclk / 8,
                        EC::InvalidArgument,
                        std::invalid_argument("Baudrate is too high"));
                    set_oversample_8x(true);
                } else
                    ELFE_ERROR(EC::InvalidArgument, std::invalid_argument("Baudrate is too high"));
            }
            baudrate = baudrate_f;
            return EC::None;
        }
        ...
        VoidResult<> HardUsart::raise_if_error() const
        {
            uint32_t sr = reg.SR;
            [[unlikely]] if (_rx_buffer_overrun) {
                _rx_buffer_overrun = false;
                ELFE_ERROR(EC::USARTOverrun, Overrun());
            }
            [[likely]] if (!(sr & (USART_SR_NE | USART_SR_PE | USART_SR_FE | USART_SR_ORE)))
                return EC::None;
            if (sr & USART_SR_NE) {
                reg.SR &= ~USART_SR_NE;
                if (!suppress_noise_error)
                    ELFE_ERROR(EC::USARTNoiseError, NoiseError());
            }
            if (sr & USART_SR_PE) {
                reg.SR &= ~USART_SR_PE;
                if (!suppress_parity_error)
                    ELFE_ERROR(EC::USARTParityError, ParityError());
            }
            if (sr & USART_SR_FE) {
                reg.SR &= ~USART_SR_FE;
                ELFE_ERROR(EC::USARTFramingError, FramingError());
            }
            if (sr & USART_SR_ORE) {
                reg.SR &= ~USART_SR_ORE;
            ...
            ELFE_ERROR(EC::USARTError, USARTError("Unknown USART error"));
        }

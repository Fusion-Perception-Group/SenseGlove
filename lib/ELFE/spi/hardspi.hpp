    ...
    namespace spi {
        ...
        class HardwareInterface : public BaseInterface {
            ...
            struct _Property : tricks::StaticProperty<T, HardwareInterface&> {
                constexpr _Property(HardwareInterface& spi)
                    : tricks::StaticProperty<T, HardwareInterface&>(spi)
                {
                }
                using tricks::StaticProperty<T, HardwareInterface&>::operator=;
            };
            struct _BaudRate : _Property<uint32_t> {
                constexpr _BaudRate(HardwareInterface& spi)
                    : _Property<uint32_t>(spi)
                {
                }
        ...
        public:
            ...
            HardwareInterface(detail::Register& reg, uint8_t order, nvic::IRQn_Type irqn)
                : reg(reg)
                , order(order)
                , irqn(irqn)
            {
            ...
            VoidResult<> turn_slave(bool yes) noexcept override final;
            bool is_slave() const noexcept override final;
            VoidResult<> set_mode(Mode mode) noexcept override final;
            Mode get_mode() const noexcept override final;
            Result<size_t> exchange_bytes(const void* tx, void* rx, size_t size) override final;
            VoidResult<> select_as_slave(bool yes) override;
            bool is_selected_as_slave() const noexcept override;
            VoidResult<> raise_if_error() const override final;

            VoidResult<> init();
            void deinit();
            VoidResult<> use_16bits(bool yes);
            bool is_using_16bits() const noexcept;
            VoidResult<> use_software_ss(bool yes);
            bool is_using_software_ss() const noexcept;
            VoidResult<> set_msb_first(bool yes);
            bool is_msb_first() const noexcept;

            VoidResult<> config_half_duplex(bool bidirectional = false, bool rxonly = false);

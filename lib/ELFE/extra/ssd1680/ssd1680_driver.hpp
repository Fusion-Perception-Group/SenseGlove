...
namespace ssd1680 {
    ...
    using namespace err::extra::ssd1680;
    ...
    public:
        ...
        VoidResult<> send_command(Command cmd, ARGS... data) const
        {
            auto r = wait_busy();
            ELFE_PROP(r, r);
            dc_pin.reset();
            select();
            r = spi.put(static_cast<uint8_t>(cmd));
            ELFE_PROP(r, r);
            dc_pin.set();
            deselect();
        ...
        template <typename... ARGS>
        VoidResult<> send_data(ARGS... data) const
        {
            ...
            dc_pin.set();
            select();
            auto ru = spi.write_bytes(buf.data(), buf.size());
        ...
        VoidResult<> init() const
        {
            ...
            ELFE_PROP(r, r);
            clock::delay(10ms);
            r = (send_command(Command::DriverOutputControl, 0x27, 0x01, 0x00) &&
        ...
        VoidResult<> send_red_data(uint8_t data) const
        {
            return send_command(Command::WriteRAMRed, data);
        }

        VoidResult<> send_bw_data(uint8_t data) const
        {
            return send_command(Command::WriteRAMBW, data);
        }
        ...
        VoidResult<> wait_busy(std::chrono::milliseconds timeout = 50000ms) const
        {
        ...
        VoidResult<> sw_reset() const
        {
            return send_command(Command::SwReset);
        }

        VoidResult<> set_cursor(uint16_t x, uint16_t y) const
        {
            auto r = send_command(Command::SetRAMXAddressCounter, x);
            ELFE_PROP(r, r);
            return send_command(Command::SetRAMYAddressCounter, y & 0xff, y >> 8);
        }

        VoidResult<> full_update() const
        {
            auto r = send_command(Command::DisplayUpdateControl2, 0xf7);
            ELFE_PROP(r, r);
            r = send_command(Command::MasterActivation);
            ELFE_PROP(r, r);
            return wait_busy();
        }

        VoidResult<> update() const
        {
            auto r = send_command(Command::DisplayUpdateControl2, 0xcc);
            ELFE_PROP(r, r);
            r = send_command(Command::MasterActivation);
            ELFE_PROP(r, r);
            return wait_busy();
        ...
        VoidResult<> power_on() const
        {
            auto r = send_command(Command::DisplayUpdateControl2, 0xf8);
            ELFE_PROP(r, r);
            r = send_command(Command::MasterActivation);
            ELFE_PROP(r, r);
            return wait_busy();

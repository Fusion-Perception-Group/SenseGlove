...
#if (defined(_ELFE_STM32F4) || defined(_ELFE_STM32H7)) && !_ELFE_STM32_USE_GENERIC && !ELFE_STM32_USE_CMSIS

#if defined(_ELFE_STM32F4)
    ...
    namespace gpio {
        ...
        PinConfig::IO Pin::_IO::getter() const
        {
            return static_cast<PinConfig::IO>(
                (owner.port.MODER >> (owner.pin * 2)) & GPIO_MODER_MODER0);
        }
        void Pin::_IO::setter(const PinConfig::IO value) const
        {
            uint32_t temp = owner.port.MODER;
            temp &= ~(GPIO_MODER_MODER0 << (owner.pin * 2));
            temp |= (static_cast<uint32_t>(value) << (owner.pin * 2));
        ...
        PinConfig::OutMode Pin::_OutMode::getter() const
        {
            return static_cast<PinConfig::OutMode>(
                (owner.port.OTYPER >> owner.pin) & GPIO_OTYPER_OT_0);
        }
        void Pin::_OutMode::setter(const PinConfig::OutMode value) const
        {
            uint32_t temp = owner.port.OTYPER;
            temp &= ~(GPIO_OTYPER_OT_0 << owner.pin);
            temp |= (static_cast<uint32_t>(value) << owner.pin);
        ...
        PinConfig::EXTIMode Pin::_EXTIMode::getter() const
        {
            return static_cast<PinConfig::EXTIMode>(
                bool(EXTI_IMR & owner.mask) | (bool(EXTI_EMR & owner.mask) << 1));
        }
        ...
        PinConfig::Trigger Pin::_Trigger::getter() const
        {
            return static_cast<PinConfig::Trigger>(
                bool(EXTI_RTSR & owner.mask) | (bool(EXTI_FTSR & owner.mask) << 1));
        }
        ...
        PinConfig::Speed Pin::_Speed::getter() const
        {
            return static_cast<PinConfig::Speed>(
                (owner.port.OSPEEDR >> (owner.pin * 2)) & GPIO_OSPEEDER_OSPEEDR0);
        }
        void Pin::_Speed::setter(const PinConfig::Speed value) const
        {
            uint32_t temp = owner.port.OSPEEDR;
            temp &= ~(GPIO_OSPEEDER_OSPEEDR0 << (owner.pin * 2));
            temp |= (static_cast<uint32_t>(value) << (owner.pin * 2));
        ...
        uint8_t Pin::_Alternate::getter() const
        {
            return static_cast<uint8_t>(
                (owner.port.AFR[owner.pin >> 3] >> ((owner.pin & 0x7) << 2)) & 0xFU);
        }
        void Pin::_Alternate::setter(const uint8_t value) const
        {
            uint32_t temp = owner.port.AFR[owner.pin >> 3];
            temp &= ~(static_cast<uint32_t>(0xFU) << ((owner.pin & 0x7) << 2));
            temp |= (static_cast<uint32_t>(value) << ((owner.pin & 0x7) << 2));
            owner.port.AFR[owner.pin >> 3] = temp;
        }
        ...
        Pin::Pin(_Port& port, const uint8_t pin)
            ...
            , pin(pin)
            , mask(1 << pin)
            , on_interrupt(hidden::interrupt_callbacks.at(pin))
        {
            port.enable_clock();
        }
        ...
        const PinConfig& Pin::load(const PinConfig& config) const
        {
            ...
            if (config.io == PinConfig::Output || config.io == PinConfig::AF) {
                // Config speed
                temp = port.OSPEEDR;
                temp &= ~(GPIO_OSPEEDER_OSPEEDR0 << (pin * 2U));
                temp |= (static_cast<uint32_t>(config.speed) << (pin * 2U));
                port.OSPEEDR = temp;
            ...
            if (config.io != PinConfig::Analog) {
                ...
                temp |= (static_cast<uint32_t>(config.pull) << (pin * 2U));
                port.PUPDR = temp;
            }

            // Config Alternate function
            if (config.io == PinConfig::AF) {
                temp = port.AFR[pin >> 3U];
                temp &= ~(static_cast<uint32_t>(0xFU) << ((pin & 0x7U) << 2U));
                temp |= (static_cast<uint32_t>(config.alternate) << ((pin & 0x7U) << 2U));
                port.AFR[pin >> 3U] = temp;
            }

            // Configure IO Direction mode (Input, Output, Alternate or Analog)
            temp = port.MODER;
            temp &= ~(GPIO_MODER_MODER0 << (pin * 2U));
            temp |= (static_cast<uint32_t>(config.io) << (pin * 2U));
            port.MODER = temp;
            ...
            if (config.exti_mode != PinConfig::NoEXTI) {
                ...
                temp &= ~(0x0FU << (4U * (pin & 0x03U)));
                temp |= (uint32_t(get_port_index(port)) << (4U * (pin & 0x03U)));
                ...
                temp = EXTI_RTSR;
                temp &= ~mask;
                if (config.trigger == PinConfig::Rising || config.trigger == PinConfig::RisingFalling) {
                    temp |= mask;
                }
                EXTI_RTSR = temp;
                ...
                temp = EXTI_FTSR;
                temp &= ~mask;
                if (config.trigger == PinConfig::Falling || config.trigger == PinConfig::RisingFalling) {
                    temp |= mask;
                }
                EXTI_FTSR = temp;
            }
        ...
        void Pin::unload() const
        {
            ...
            temp = SYSCFG->EXTICR[pin >> 2U];
            temp &= ~(0x0FU << (4U * (pin & 0x03U)));
            if (temp == (uint32_t(get_port_index(port)) << (4U * (pin & 0x03U)))) {
                // Clear EXTI line configuration
                EXTI_IMR &= ~uint32_t(pin);
                EXTI_EMR &= ~uint32_t(pin);

                // Clear Rising Falling edge configuration
                EXTI_RTSR &= ~uint32_t(pin);
                EXTI_FTSR &= ~uint32_t(pin);
            ...
            port.AFR[pin >> 3U] &= ~(uint32_t(0xFU) << ((pin & 0x7U) << 2U));
            port.PUPDR &= ~(GPIO_PUPDR_PUPDR0 << (pin * 2U));
            port.OTYPER &= ~(GPIO_OTYPER_OT_0 << pin);
            port.OSPEEDR &= ~(GPIO_OSPEEDER_OSPEEDR0 << (pin * 2U));
        }
...
#endif // defined(_ELFE_STM32F4) || defined(_ELFE_STM32H7)

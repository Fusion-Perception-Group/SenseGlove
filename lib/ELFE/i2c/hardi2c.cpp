...
namespace stm32 {
    namespace i2c {
        ...
        Result<bool> HardMaster::select(addr_t address, const bool read) const
        {
            reg.SR1 &= ~I2C_SR1_AF;
            reg.CR1 |= I2C_CR1_PE;
            ...
            for (int i = 0; (reg.SR2 & I2C_SR2_BUSY); ++i) {
                if (i == 100)
                    ELFE_ERROR(Result<bool>(false, EC::I2CBusBusy), BusBusy());
            }
        ...
        void HardMaster::end() const noexcept
        {
            reg.CR1 |= I2C_CR1_STOP;
        }
        Result<bool> HardMaster::write_byte(uint8_t data) const
        {
            reg.SR1 &= ~I2C_SR1_AF;
            while (not(reg.SR1 & I2C_SR1_TXE)) {
                auto r = raise_if_error();
        ...
        Result<uint8_t> HardMaster::read_byte(bool acknowledge) const
        {
            reg.SR1 &= ~I2C_SR1_AF;
            if (acknowledge)
                reg.CR1 |= I2C_CR1_ACK;
        ...
        VoidResult<> HardMaster::raise_if_error(const bool end) const
        {
            if (reg.SR1 & (I2C_SR1_PECERR | I2C_SR1_TIMEOUT | I2C_SR1_OVR | I2C_SR1_ARLO | I2C_SR1_BERR)) {
                ...
                if (reg.SR1 & I2C_SR1_PECERR) {
                    ...
                    ELFE_ERROR(EC::I2CValidationError, ValidationError());
                } else if (reg.SR1 & I2C_SR1_TIMEOUT) {
                    reg.SR1 &= ~I2C_SR1_TIMEOUT;
                    ELFE_ERROR(EC::I2CTimeout, I2CTimeout());
                } else if (reg.SR1 & I2C_SR1_OVR) {
                    reg.SR1 &= ~I2C_SR1_OVR;
                    ELFE_ERROR(EC::I2COverrun, Overrun());
                    ...
                    ELFE_ERROR(EC::I2CBusError, BusError());
                }
            }
            return EC::None;
        }

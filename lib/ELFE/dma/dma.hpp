    ...
    namespace dma {
        ...
        public:
            ...
            public:
                ...
                Stream(BaseDMA& dma, uint8_t order, detail::_DMAStreamReg& reg, nvic::IRQn_Type irqn)
                    : dma(dma)
                    , order(order)
                    , reg(reg)
                    , irqn(irqn)
                ...
                VoidResult<> select_channel(uint8_t channel) const
                {
                    ELFE_ERROR_IF(
                        channel > 7,
                        EC::InvalidArgument, std::invalid_argument("channel must be between 0 and 7"));
                    reg.CR &= ~(0x7 << 25);
                    reg.CR |= (channel << 25);
                    return EC::None;
                }
                ...
                volatile void* get_source_addr() const noexcept
                {
                ...
                volatile void* get_dest_addr() const noexcept
                {
                    if (get_direction() == Direction::Ram2Peri)
                        return reg.PAR;
                    else
                ...
                VoidResult<> set_src_addr(volatile void* value) const
                {
                    const bool src_is_ram = is_ram_addr(value);
                    volatile void* const dst_addr = get_dest_addr();
                    const bool dst_is_ram = is_ram_addr(dst_addr);
                    VoidResult<> r;
                    if (not src_is_ram) {
                        reg.PAR = value;
                        reg.M0AR = dst_addr;
                        r = set_direction(Direction::Peri2Ram);
                    } else if (dst_is_ram) {
                        reg.PAR = value;
                        reg.M0AR = dst_addr;
                        r = set_direction(Direction::Ram2Ram);
                    } else {
                        reg.M0AR = value;
                        reg.PAR = dst_addr;
                        r = set_direction(Direction::Ram2Peri);
                    }
                    return r;
                }

                VoidResult<> set_dest_addr(volatile void* value) const
                {
                    const bool dst_is_ram = is_ram_addr(value);
                    volatile void* const src_addr = get_source_addr();
                ...
                virtual VoidResult<> set_direction(Direction direction) const
                {
                    ELFE_ERROR_IF(
                        direction == Direction::Ram2Ram,
                        EC::InvalidArgument, std::invalid_argument("direction cannot be Ram2Ram"));
                    reg.CR &= ~(0x3 << 6);
                ...
                VoidResult<> raise_if_error() const
                {
                    ...
                    if (sr & TRANS_ERR_MASK) {
                        clr = TRANS_ERR_MASK;
                        ELFE_ERROR(EC::DMATransferError, TransferError());
                    }
                    if (sr & FIFO_ERR_MASK) {
                        clr = FIFO_ERR_MASK;
                        ELFE_ERROR(EC::DMAFIFOError, FIFOError());
                    }
                    if (sr & DIR_ERR_MASK) {
                        clr = DIR_ERR_MASK;
                        ELFE_ERROR(EC::DMADirectModeError, DirectModeError());
                    }
                    return EC::None;
                }
            ...
            public:
                StreamM2M(BaseDMA& dma, uint8_t order, detail::_DMAStreamReg& reg, nvic::IRQn_Type irqn)
                    : Stream(dma, order, reg, irqn)
                {
                }

                VoidResult<> set_direction(Direction direction) const override
                {
                    reg.CR &= ~(0x3 << 6);
                    reg.CR |= (static_cast<uint8_t>(direction) << 6);
                    return EC::None;
            ...
            BaseDMA(const uint8_t order, detail::_DMAReg& reg)
                : order(order)
                , reg(reg)
            {
            }

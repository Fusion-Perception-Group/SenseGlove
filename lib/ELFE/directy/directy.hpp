...
namespace directy {
    ...
    namespace helper {
        inline constexpr uint8_t int_sqrt16(const uint16_t x) noexcept
        {
            uint8_t res = 0;
            uint8_t add = 0x80;
            for (int i = 8; i--;) {
        ...
        inline constexpr uint16_t int_sqrt32(const uint32_t x) noexcept
        {
            if (x <= 0xFFFF)
                return int_sqrt16(x);
            uint16_t res = 0;
        ...
        inline constexpr uint32_t int_sqrt64(const uint64_t x) noexcept
        {
            if (x <= 0xFFFFFFFF)
                return int_sqrt32(x);
            uint32_t res = 0;
        ...
        inline constexpr T int_sqrt(T x) noexcept
            requires(4 >= sizeof(T))
        {
            return int_sqrt32(x);
        }
         ...
         */
        template <std::unsigned_integral T>
        inline constexpr T inv_bits(T x) noexcept
        {
            size_t n = sizeof(T) * 8;
         ...
         */
        template <std::unsigned_integral T>
        inline constexpr T fix_point_mul(T a, T b) noexcept
        {
         ...
         */
        template <std::unsigned_integral T>
        inline constexpr T fix_point_div(T a, T b) noexcept
        {
    ...
    public:
        ...
        template <typename T, typename... ARGS>
        T& create_child_at(size_t index, ARGS&&... args)
            requires std::derived_from<T, Node>;
    };
    ...
    protected:
        ...
        constexpr Node() {};
        virtual VoidResult<> _draw_this_impl(const Context& ctx) { return EC::None; }

        static VoidResult<> _draw_pixels_with_ctx(const Context& ctx,
        ...
        static VoidResult<> _draw_pixel_with_ctx(const Context& ctx, coord_t x, coord_t y, uni_t color);
    ...
    public:
        ...
        Result<Context> draw_this(const Context& ctx)
        {
            Context new_ctx = ctx;
            ...
            default:
                ELFE_ERROR(Result<Context>(ctx, EC::UnknownError), std::runtime_error("Invalid transform reference"));
            }
            new_ctx.parent = operator&();
            auto r = _draw_this_impl(new_ctx);
            ELFE_PROP(r, Result<Context>(new_ctx, r.error));
            return new_ctx;
        }
        VoidResult<> draw(const Context& ctx)
        {
            if (not enabled)
                return EC::None;
            auto r = draw_this(ctx);
            ELFE_PROP(r, r.error);
            auto new_ctx = r.value;
            for (auto& child : children) {
                auto r = child->draw(new_ctx);
                ELFE_PROP(r, r);
            }
            return EC::None;
        }
    ...
    class BaseCanvas : public BaseNode {
        constexpr VoidResult<> _draw_seg(
            ...
            const uni_t color, const uni_t alpha = PX_MAX)
        {
            ...
            if (!length) {
                r = uni_set_pixel(x0, y0, color, alpha);
                ELFE_PROP(r, r);
                return EC::None;
            }

            [[unlikely]] if (!dx or !dy) {
                // treat horizontal and vertical line as rectangle
                r = uni_draw_rect(x0, y0, x1, y1, color, alpha); // faster
                ELFE_PROP(r, r);
                return EC::None;
            }
    ...
    public:
        ...
        constexpr virtual coord_t get_width() const noexcept = 0;
        constexpr virtual coord_t get_height() const noexcept = 0;
        constexpr virtual uint8_t get_pixel_bits() const noexcept = 0;

        virtual VoidResult<> draw(
            ...
            uint64_t timestamp_ms = stm32::clock::get_systick_ms())
        {
            ...
            for (auto& child : children) {
                auto r = child->draw(ctx);
                ELFE_PROP(r, r);
            }
            return EC::None;
        }

        constexpr virtual Result<uni_t> uni_get_pixel(coord_t x, coord_t y) const = 0;
        constexpr virtual VoidResult<> uni_set_pixel(
            coord_t x, coord_t y, uni_t color, uni_t alpha = PX_MAX)
            = 0;

        constexpr virtual VoidResult<> uni_fill(uni_t color)
        {
            const auto WIDTH = get_width(), HEIGHT = get_height();
            for (coord_t y = 0; y < HEIGHT; ++y) {
                for (coord_t x = 0; x < WIDTH; ++x) {
                    auto r = uni_set_pixel(x, y, color);
                    ELFE_PROP(r, r);
                }
            }
            return EC::None;
        ...
        constexpr virtual VoidResult<> clear()
        {
            auto r = uni_fill(PX_MIN);
            ELFE_PROP(r, r);
            return EC::None;
        }
         ...
         * @param alpha
         * @param type
         * @return constexpr VoidResult<>
         * @throw std::invalid_argument
         */
        constexpr virtual VoidResult<> uni_draw_line(
            coord_t x0, coord_t y0, coord_t x1, coord_t y1,
            ...
            LineType type = LineType::Segment)
        {
            ...
            if (x0 >= 0 and x0 < WIDTH
                ...
                r = _draw_seg(x0, y0, x1, y1, color);
                ELFE_PROP(r, r);
                return EC::None;
            }

            ELFE_ERROR_IF(
                not dx and not dy, EC::InvalidArgument,
                std::invalid_argument("Unit length line only allowed in line segment mode"));
            ...
            if (inter_cnt != 2)
                return EC::None; // line outside the canvas
         ...
         * @param color
         */
        constexpr virtual VoidResult<> uni_draw_rect(
            coord_t x0, coord_t y0, coord_t x1, coord_t y1,
            uni_t color = PX_MAX, uni_t alpha = PX_MAX)
        {
            VoidResult<> r;
            ...
            if (has_t)
                for (coord_t x = x0; x <= x1; ++x) {
                    r = uni_set_pixel(x, y1, color, alpha);
                    ELFE_PROP(r, r);
                }
            if (has_b)
                for (coord_t x = x0; x <= x1; ++x) {
                    r = uni_set_pixel(x, y0, color, alpha);
                    ELFE_PROP(r, r);
                }
            if (has_l)
                for (coord_t y = y0; y <= y1; ++y) {
                    r = uni_set_pixel(x0, y, color, alpha);
                    ELFE_PROP(r, r);
                }
            if (has_r)
                for (coord_t y = y0; y <= y1; ++y) {
                    r = uni_set_pixel(x1, y, color, alpha);
                    ELFE_PROP(r, r);
                }
            return EC::None;
        }
        constexpr virtual VoidResult<> uni_draw_filled_rect(
            coord_t x0, coord_t y0, coord_t x1, coord_t y1,
            uni_t color = PX_MAX, uni_t alpha = PX_MAX)
        {
            const auto WIDTH = get_width(), HEIGHT = get_height();
            ...
            for (coord_t y = y0; y <= y1; ++y) {
                for (coord_t x = x0; x <= x1; ++x) {
                    auto r = uni_set_pixel(x, y, color, alpha);
                    ELFE_PROP(r, r);
                }
            }
            return EC::None;
         ...
         * @param color
         */
        constexpr virtual VoidResult<> uni_draw_circle(
            ...
            uni_t color = PX_MAX, uni_t alpha = PX_MAX)
        {
            ...
            coord_t err = 1 - x;
            while (x >= y) {
                auto r = uni_set_pixel(x0 + x, y0 + y, color, alpha);
                ELFE_PROP(r, r);
                r = uni_set_pixel(x0 + y, y0 + x, color, alpha);
                ELFE_PROP(r, r);
                r = uni_set_pixel(x0 - y, y0 + x, color, alpha);
                ELFE_PROP(r, r);
                r = uni_set_pixel(x0 - x, y0 + y, color, alpha);
                ELFE_PROP(r, r);
                r = uni_set_pixel(x0 - x, y0 - y, color, alpha);
                ELFE_PROP(r, r);
                r = uni_set_pixel(x0 - y, y0 - x, color, alpha);
    ...
    inline VoidResult<> Node::_draw_pixel_with_ctx(const Context& ctx, coord_t x, coord_t y, uni_t color)
    {
        ...
        if (x < 0 or x >= ctx.canvas.get_width()
            ...
            return EC::None;
        auto r = ctx.canvas.uni_set_pixel(x, y, color, ctx.alpha);
        ELFE_PROP(r, r);
        return EC::None;
    }

    inline VoidResult<> Node::_draw_pixels_with_ctx(const Context& ctx,
        std::function<std::tuple<Pixel, bool>()> point_gen)
    {
        auto cos_rot = 1.0f;
        auto sin_rot = 0.0f;
    ...
    public:
        Mat_t pixels {};
        constexpr coord_t get_width() const noexcept override { return W; }
        constexpr coord_t get_height() const noexcept override { return H; }
        constexpr uint8_t get_pixel_bits() const noexcept override { return BIT; }
         ...
         * @throw std::invalid_argument
         */
        constexpr VoidResult<> uni_fill(uni_t color) override
        {
            color >>= (sizeof(uni_t) * 8 - BIT); // rescale to pixel range
            for (auto& row : pixels) {
                auto r = row.fill(color); // rescale to pixel range
                ELFE_PROP(r, r);
            }
            return EC::None;
        }
         ...
         * @param x
         * @param y
         * @return constexpr Result<uni_t>
         * @throw std::invalid_argument
         */
        constexpr Result<uni_t> uni_get_pixel(const coord_t x, const coord_t y) const override
        {
            ELFE_ERROR_IF(
                x < 0 or x >= static_cast<coord_t>(W) or y < 0 or y >= static_cast<coord_t>(H),
                Result<uni_t>(PX_MIN, EC::InvalidArgument), std::invalid_argument("coordinates out of range"));
         ...
         * @throw std::invalid_argument
         */
        constexpr VoidResult<> uni_set_pixel(
            const coord_t x, const coord_t y,
            uni_t color = PX_MAX, const uni_t alpha = PX_MAX) override
        {
            ELFE_ERROR_IF(
                x < 0 or x >= static_cast<coord_t>(W) or y < 0 or y >= static_cast<coord_t>(H),
                EC::InvalidArgument, std::invalid_argument("coordinates out of range"));
            if (alpha != PX_MAX) {
        ...
        constexpr Result<PX_T> get_pixel(const coord_t x, const coord_t y) const
        {
            ELFE_ERROR_IF(
                x < 0 or x >= static_cast<coord_t>(W) or y < 0 or y >= static_cast<coord_t>(H),
                Result<PX_T>(PX_MIN, EC::InvalidArgument), std::invalid_argument("coordinates out of range"));
            return pixels[y][x];
        }
        constexpr VoidResult<> set_pixel(const coord_t x, const coord_t y, const PX_T color)
        {
            ELFE_ERROR_IF(
                x < 0 or x >= static_cast<coord_t>(W) or y < 0 or y >= static_cast<coord_t>(H),
                EC::InvalidArgument, std::invalid_argument("coordinates out of range"));
    ...
    public:
        ...
        constexpr Result<uni_t> uni_get_pixel(
            const coord_t x, const coord_t y) const override
        {
            ELFE_ERROR_IF(
                x < 0 or x >= static_cast<coord_t>(W)
        ...
        constexpr Result<uint8_t> get_pixel(const coord_t x, const coord_t y) const
        {
            ELFE_ERROR_IF(
                x < 0 or x >= static_cast<coord_t>(W)
                    or y < 0 or y >= static_cast<coord_t>(H),
                Result<uint8_t>(0, EC::InvalidArgument), std::invalid_argument("coordinates out of range"));
            return (y % 2) ? (pixels[y / 2][x] & 0x0F) : (pixels[y / 2][x] >> 4);
        }
        constexpr VoidResult<> set_pixel(const coord_t x, const coord_t y, const uint8_t color)
        {
            ELFE_ERROR_IF(
                x < 0 or x >= static_cast<coord_t>(W)
                    or y < 0 or y >= static_cast<coord_t>(H),
    ...
    class Canvas1B : public BaseCanvas {
        using Row_t = std::array<uint8_t, W>;
        using Mat_t = std::array<Row_t, (H + 7) / 8>;
    ...
    public:
            ...
            uni_t color = PX_MAX, const uni_t alpha = PX_MAX) override
        {
            ELFE_ERROR_IF(
                ...
                EC::InvalidArgument, std::invalid_argument("coordinates out of range"));
            if (alpha != PX_MAX) {
                uni_t bg_color = uni_get_pixel(x, y).value;
                color = helper::fix_point_mul(color, alpha)
                    + helper::fix_point_mul(bg_color, PX_MAX - alpha);
            }
            uni_t pixel = color >> (sizeof(uni_t) * 8 - BIT); // rescale to pixel range
        ...
        constexpr Result<bool> get_pixel(const coord_t x, const coord_t y) const
        {
            ELFE_ERROR_IF(
                x < 0 or x >= static_cast<coord_t>(W)
                    or y < 0 or y >= static_cast<coord_t>(H),
                Result<bool>(false, EC::InvalidArgument), std::invalid_argument("coordinates out of range"));
            const unsigned shift = y % 8;
            return (pixels[y / 8][x] >> shift) & 0x01;
        }
        constexpr VoidResult<> set_pixel(const coord_t x, const coord_t y, const bool color)
        {
            ELFE_ERROR_IF(
                x < 0 or x >= static_cast<coord_t>(W)
                    or y < 0 or y >= static_cast<coord_t>(H),
    ...
    protected:
        ...
        constexpr LineNode(
            ...
            , color(color)
            , type(type) {};
        VoidResult<> _draw_this_impl(const Context& ctx) override
        {
            ...
            auto r = ctx.canvas.uni_draw_line(
                ctx.x + _x0, ctx.y + _y0,
                    ...
                    > x1 - static_cast<coord_t>(margin)) // line exceeding
                {
                    ...
                    case ExceedAction::Truncate:
                        for (; i < MAX; ++i) {
                            if (content[i] == '\n') { // find next new line
                                if (++i >= MAX)
                                    return EC::None; // no more content
                                c = content[i];
                                break;
                            }
                            return EC::None;
                    ...
                    case ExceedAction::Wrap:
                        ...
                        break;
                    default:
                        ELFE_ERROR(EC::InvalidArgument, std::invalid_argument("Invalid exceed action"));
                    }
                }
                    ...
                    for (coord_t col = 0; col < 8; ++col) {
                        ...
                        for (coord_t row = 0; row < 8; ++row) {
                            ...
                            if (px_x >= x0 and px_x < x1
                                ...
                                auto r = _draw_pixel_with_ctx(ctx, px_x, px_y, color);
                                ELFE_PROP(r, r);
                            }
                        }
                    }
